...
## 9. Correção da Tabela e Funções de Números Sequenciais (Filas Isoladas por Categoria)

Este script resolve o problema onde números cancelados de uma categoria (ex: 1.8) eram incorretamente atribuídos a outra categoria (ex: 1.2), garantindo que a fila de `numeros_disponiveis` e as funções RPC filtrem estritamente por `categoria_id`.
